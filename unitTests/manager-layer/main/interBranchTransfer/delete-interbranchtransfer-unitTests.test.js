...
describe("DeleteInterBranchTransfer", () => {
  let ControllerClass;
  let req;
  ...
  beforeEach(() => {
    ...
    ControllerClass = proxyquire(
      "../../../../src/manager-layer/main/InterBranchTransfer/delete-interbranchtransfer",
      {
        "./InterBranchTransferManager": class {
  ...
  it("should initialize controller with correct base properties", () => {
    ...
    expect(instance.options.name).to.equal("deleteInterBranchTransfer");
    expect(instance.options.controllerType).to.equal("rest");
    expect(instance.options.crudType).to.equal("delete");
    expect(instance.dataName).to.equal("interBranchTransfer");
  });
});
