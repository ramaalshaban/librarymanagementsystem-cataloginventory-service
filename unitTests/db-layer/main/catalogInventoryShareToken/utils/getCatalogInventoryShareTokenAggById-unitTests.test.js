const { expect } = require("chai");
const sinon = require("sinon");
const proxyquire = require("proxyquire");
const { Op } = require("sequelize");

describe("getCatalogInventoryShareTokenAggById module", () => {
  let sandbox;
  let getCatalogInventoryShareTokenAggById;
  let CatalogInventoryShareTokenStub;

  const fakeId = "uuid-123";
  const fakeData = { id: fakeId, name: "Test CatalogInventoryShareToken" };

  beforeEach(() => {
    sandbox = sinon.createSandbox();

    CatalogInventoryShareTokenStub = {
      findOne: sandbox.stub().resolves({ getData: () => fakeData }),
      findAll: sandbox
        .stub()
        .resolves([
          { getData: () => ({ id: "1", name: "Item 1" }) },
          { getData: () => ({ id: "2", name: "Item 2" }) },
        ]),
      getCqrsJoins: sandbox.stub().resolves(),
    };

    getCatalogInventoryShareTokenAggById = proxyquire(
      "../../../../../src/db-layer/main/CatalogInventoryShareToken/utils/getCatalogInventoryShareTokenAggById",
      {
        models: { CatalogInventoryShareToken: CatalogInventoryShareTokenStub },
        common: {
          HttpServerError: class HttpServerError extends Error {
            constructor(msg, details) {
              super(msg);
              this.name = "HttpServerError";
              this.details = details;
            }
          },
        },
        sequelize: { Op },
      },
    );
  });

  afterEach(() => sandbox.restore());

  describe("getCatalogInventoryShareTokenAggById", () => {
    it("should return getData() with includes for single ID", async () => {
      const result = await getCatalogInventoryShareTokenAggById(fakeId);
      expect(result).to.deep.equal(fakeData);
      sinon.assert.calledOnce(CatalogInventoryShareTokenStub.findOne);
      sinon.assert.calledOnce(CatalogInventoryShareTokenStub.getCqrsJoins);
    });

    it("should return mapped getData() for array of IDs", async () => {
      const result = await getCatalogInventoryShareTokenAggById(["1", "2"]);
      expect(result).to.deep.equal([
        { id: "1", name: "Item 1" },
        { id: "2", name: "Item 2" },
      ]);
      sinon.assert.calledOnce(CatalogInventoryShareTokenStub.findAll);
      sinon.assert.calledOnce(CatalogInventoryShareTokenStub.getCqrsJoins);
    });

    it("should return null if not found for single ID", async () => {
      CatalogInventoryShareTokenStub.findOne.resolves(null);
      const result = await getCatalogInventoryShareTokenAggById(fakeId);
      expect(result).to.equal(null);
    });

    it("should return empty array if input is array but no results", async () => {
      CatalogInventoryShareTokenStub.findAll.resolves([]);
      const result = await getCatalogInventoryShareTokenAggById(["nope"]);
      expect(result).to.deep.equal([]);
    });

    it("should return undefined if getData returns undefined in array items", async () => {
      CatalogInventoryShareTokenStub.findAll.resolves([
        { getData: () => undefined },
        { getData: () => undefined },
      ]);
      const result = await getCatalogInventoryShareTokenAggById(["1", "2"]);
      expect(result).to.deep.equal([undefined, undefined]);
    });

    it("should return undefined if getData returns undefined in single ID", async () => {
      CatalogInventoryShareTokenStub.findOne.resolves({
        getData: () => undefined,
      });
      const result = await getCatalogInventoryShareTokenAggById(fakeId);
      expect(result).to.be.undefined;
    });

    it("should throw HttpServerError on unexpected error (findOne)", async () => {
      CatalogInventoryShareTokenStub.findOne.rejects(new Error("fail"));
      try {
        await getCatalogInventoryShareTokenAggById(fakeId);
        throw new Error("Expected HttpServerError");
      } catch (err) {
        expect(err.name).to.equal("HttpServerError");
        expect(err.message).to.equal(
          "errMsg_dbErrorWhenRequestingCatalogInventoryShareTokenAggById",
        );
        expect(err.details.message).to.equal("fail");
      }
    });

    it("should throw HttpServerError on unexpected error (findAll)", async () => {
      CatalogInventoryShareTokenStub.findAll.rejects(new Error("all fail"));
      try {
        await getCatalogInventoryShareTokenAggById(["1"]);
        throw new Error("Expected HttpServerError");
      } catch (err) {
        expect(err.name).to.equal("HttpServerError");
        expect(err.message).to.equal(
          "errMsg_dbErrorWhenRequestingCatalogInventoryShareTokenAggById",
        );
        expect(err.details.message).to.equal("all fail");
      }
    });

    it("should throw HttpServerError if getCqrsJoins fails", async () => {
      CatalogInventoryShareTokenStub.getCqrsJoins.rejects(
        new Error("joins fail"),
      );
      try {
        await getCatalogInventoryShareTokenAggById(fakeId);
        throw new Error("Expected HttpServerError");
      } catch (err) {
        expect(err.name).to.equal("HttpServerError");
        expect(err.message).to.equal(
          "errMsg_dbErrorWhenRequestingCatalogInventoryShareTokenAggById",
        );
        expect(err.details.message).to.equal("joins fail");
      }
    });
  });
});
