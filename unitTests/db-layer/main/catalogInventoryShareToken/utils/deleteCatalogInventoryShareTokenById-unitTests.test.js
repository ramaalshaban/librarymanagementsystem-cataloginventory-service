...
describe("deleteCatalogInventoryShareTokenById module", () => {
  let sandbox;
  let deleteCatalogInventoryShareTokenById;
  ...
  beforeEach(() => {
    ...
    mockDocInstance = {
      ...
      getData: () => ({
        id: fakeId,
        name: "Deleted CatalogInventoryShareToken",
      }),
    };
    ...
    deleteCatalogInventoryShareTokenById = proxyquire(
      "../../../../../src/db-layer/main/CatalogInventoryShareToken/utils/deleteCatalogInventoryShareTokenById",
      {
  ...
  describe("deleteCatalogInventoryShareTokenById", () => {
    it("should delete CatalogInventoryShareToken and index removal in Elastic", async () => {
      const result = await deleteCatalogInventoryShareTokenById(fakeId);
    ...
    it("should support object as ID input", async () => {
      ...
      CatalogInventoryShareTokenStub.findOne.resolves(localMock);

      const result = await deleteCatalogInventoryShareTokenById({ id: fakeId });
    ...
    it("should throw BadRequestError if no id is provided", async () => {
      try {
        await deleteCatalogInventoryShareTokenById(undefined);
    ...
    it("should throw NotFoundError if record is not found", async () => {
      ...
      try {
        await deleteCatalogInventoryShareTokenById(fakeId);
    ...
    it("should wrap ElasticIndexer.deleteData() error with HttpServerError", async () => {
      ...
      const result = await deleteCatalogInventoryShareTokenById(fakeId).catch(
        (err) => err,
      );
    ...
    it("should accept numeric ID as valid input", async () => {
      ...
      const result = await deleteCatalogInventoryShareTokenById(numericId);
      expect(result).to.deep.equal({
        id: numericId,
        name: "Deleted CatalogInventoryShareToken",
      });
