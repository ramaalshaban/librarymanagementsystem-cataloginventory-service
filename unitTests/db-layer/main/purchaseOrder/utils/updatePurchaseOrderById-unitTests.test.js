...
describe("updatePurchaseOrderById module", () => {
  let sandbox;
  let updatePurchaseOrderById;
  let PurchaseOrderStub, ElasticIndexerStub;

  const fakeId = "uuid-123";
  const fakeUpdatedData = { id: fakeId, name: "Updated PurchaseOrder" };
  ...
  beforeEach(() => {
    ...
    updatePurchaseOrderById = proxyquire(
      "../../../../../src/db-layer/main/PurchaseOrder/utils/updatePurchaseOrderById",
      {
  ...
  describe("updatePurchaseOrderById", () => {
    it("should update record by direct ID", async () => {
      const result = await updatePurchaseOrderById(fakeId, { name: "Updated" });
    ...
    it("should extract ID from dataClause if not explicitly provided", async () => {
      const result = await updatePurchaseOrderById(undefined, {
        id: fakeId,
        name: "Updated",
      });
      expect(result).to.deep.equal(fakeUpdatedData);
    ...
    it("should support object-as-ID input", async () => {
      const result = await updatePurchaseOrderById(
        { id: fakeId },
        { name: "Updated" },
      );
      expect(result).to.deep.equal(fakeUpdatedData);
    ...
    it("should throw BadRequestError if no ID is provided", async () => {
      try {
        await updatePurchaseOrderById(undefined, {});
    ...
    it("should throw NotFoundError if no existing record is found", async () => {
      ...
      try {
        await updatePurchaseOrderById(fakeId, { name: "Updated" });
        throw new Error("Expected error");
      } catch (err) {
        expect(err.name).to.equal("HttpServerError");
    ...
    it("should throw HttpServerError if findOne fails", async () => {
      PurchaseOrderStub.findOne.rejects(new Error("DB fail"));
      try {
        await updatePurchaseOrderById(fakeId, { name: "Updated" });
        throw new Error("Expected error");
      } catch (err) {
