...
describe("deleteBookById module", () => {
  let sandbox;
  let deleteBookById;
  ...
  beforeEach(() => {
    ...
    mockDocInstance = {
      ...
      getData: () => ({ id: fakeId, name: "Deleted Book" }),
    };
    ...
    deleteBookById = proxyquire(
      "../../../../../src/db-layer/main/Book/utils/deleteBookById",
      {
  ...
  describe("deleteBookById", () => {
    it("should delete Book and index removal in Elastic", async () => {
      const result = await deleteBookById(fakeId);

      expect(result).to.deep.equal({ id: fakeId, name: "Deleted Book" });
    ...
    it("should support object as ID input", async () => {
      ...
      BookStub.findOne.resolves(localMock);

      const result = await deleteBookById({ id: fakeId });
    ...
    it("should throw BadRequestError if no id is provided", async () => {
      try {
        await deleteBookById(undefined);
    ...
    it("should wrap internal errors with HttpServerError", async () => {
      BookStub.findOne.rejects(new Error("unexpected error"));
    ...
    it("should wrap ElasticIndexer.deleteData() error with HttpServerError", async () => {
      ...
      const result = await deleteBookById(fakeId).catch((err) => err);
    ...
    it("should accept numeric ID as valid input", async () => {
      ...
      const localMock = {
        id: numericId,
        update: sandbox.stub().resolves(),
        getData: () => ({ id: numericId, name: "Deleted Book" }),
      };
      BookStub.findOne.resolves(localMock);

      const result = await deleteBookById(numericId);
      expect(result).to.deep.equal({ id: numericId, name: "Deleted Book" });
      sinon.assert.calledOnce(localMock.update);
    });
  });
});
