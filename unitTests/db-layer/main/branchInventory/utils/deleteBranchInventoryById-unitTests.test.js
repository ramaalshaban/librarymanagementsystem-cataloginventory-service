...
describe("deleteBranchInventoryById module", () => {
  let sandbox;
  let deleteBranchInventoryById;
  ...
  beforeEach(() => {
    ...
    mockDocInstance = {
      ...
      getData: () => ({ id: fakeId, name: "Deleted BranchInventory" }),
    };
    ...
    deleteBranchInventoryById = proxyquire(
      "../../../../../src/db-layer/main/BranchInventory/utils/deleteBranchInventoryById",
      {
  ...
  describe("deleteBranchInventoryById", () => {
    it("should delete BranchInventory and index removal in Elastic", async () => {
      const result = await deleteBranchInventoryById(fakeId);

      expect(result).to.deep.equal({
        id: fakeId,
        name: "Deleted BranchInventory",
      });
    ...
    it("should support object as ID input", async () => {
      ...
      BranchInventoryStub.findOne.resolves(localMock);

      const result = await deleteBranchInventoryById({ id: fakeId });
    ...
    it("should throw BadRequestError if no id is provided", async () => {
      try {
        await deleteBranchInventoryById(undefined);
    ...
    it("should throw NotFoundError if record is not found", async () => {
      ...
      try {
        await deleteBranchInventoryById(fakeId);
    ...
    it("should wrap internal errors with HttpServerError", async () => {
      BranchInventoryStub.findOne.rejects(new Error("unexpected error"));
    ...
    it("should wrap ElasticIndexer.deleteData() error with HttpServerError", async () => {
      ...
      const result = await deleteBranchInventoryById(fakeId).catch(
        (err) => err,
      );
    ...
    it("should accept numeric ID as valid input", async () => {
      ...
      const localMock = {
        id: numericId,
        update: sandbox.stub().resolves(),
        getData: () => ({ id: numericId, name: "Deleted BranchInventory" }),
      };
      BranchInventoryStub.findOne.resolves(localMock);

      const result = await deleteBranchInventoryById(numericId);
      expect(result).to.deep.equal({
        id: numericId,
        name: "Deleted BranchInventory",
      });
