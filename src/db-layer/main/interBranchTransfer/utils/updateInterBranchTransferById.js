...
const updateInterBranchTransferById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const [rowsCount, [dbDoc]] = await InterBranchTransfer.update(
      dataClause,
      options,
    );
    if (!dbDoc) {
...
module.exports = updateInterBranchTransferById;
