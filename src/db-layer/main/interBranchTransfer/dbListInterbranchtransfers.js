...
class DbListInterbranchtransfersCommand extends DBGetListMongooseCommand {
  constructor(input) {
    super(input);
  ...
  createQuery() {
    ...
    return InterBranchTransfer.find(this.whereClause);
  }
  ...
  async setPaginationTotalRowCount() {
    this.paginationTotalRowCount = await InterBranchTransfer.countDocuments(
      this.whereClause,
    );
  }
