...
const updateInventoryAuditLogById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await InventoryAuditLog.findOne({
      _id: id,
      isActive: true,
    });
    ...
    const dbDoc = await InventoryAuditLog.findOneAndUpdate(
      whereClause,
      dataClause,
      options,
    );
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenUpdatingInventoryAuditLogById",
      err,
    );
  }
};

module.exports = updateInventoryAuditLogById;
