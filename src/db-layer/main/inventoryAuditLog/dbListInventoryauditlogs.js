...
class DbListInventoryauditlogsCommand extends DBGetListMongooseCommand {
  constructor(input) {
    super(input);
  ...
  createQuery() {
    ...
    return InventoryAuditLog.find(this.whereClause);
  }
  ...
  async setPaginationTotalRowCount() {
    this.paginationTotalRowCount = await InventoryAuditLog.countDocuments(
      this.whereClause,
    );
  }
