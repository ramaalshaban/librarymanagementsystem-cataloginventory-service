...
const updateCatalogInventoryShareTokenById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const dbDoc = await CatalogInventoryShareToken.findOneAndUpdate(
      whereClause,
      dataClause,
      options,
    );
...
module.exports = updateCatalogInventoryShareTokenById;
