...
class DbListBranchinventoriesCommand extends DBGetListMongooseCommand {
  constructor(input) {
    super(input);
  ...
  createQuery() {
    ...
    return BranchInventory.find(this.whereClause);
  }
  ...
  async setPaginationTotalRowCount() {
    this.paginationTotalRowCount = await BranchInventory.countDocuments(
      this.whereClause,
    );
  }
