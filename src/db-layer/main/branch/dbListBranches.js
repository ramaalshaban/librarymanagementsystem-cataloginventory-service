...
class DbListBranchesCommand extends DBGetListMongooseCommand {
  constructor(input) {
    super(input);
  ...
  createQuery() {
    ...
    return Branch.find(this.whereClause);
  }
  ...
  async setPaginationTotalRowCount() {
    this.paginationTotalRowCount = await Branch.countDocuments(
      this.whereClause,
    );
  }
