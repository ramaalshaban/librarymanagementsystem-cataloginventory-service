...
class ListPurchaseOrdersMcpController extends CatalogInventoryMcpController {
  constructor(params) {
    super("listPurchaseOrders", "listpurchaseorders", params);
    this.dataName = "purchaseOrders";
    this.crudType = "getList";
  }

  createApiManager() {
    return new ListPurchaseOrdersManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    description:
      "List/search branch purchase/acquisition order records (by branch, status).",
    parameters: ListPurchaseOrdersMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const listPurchaseOrdersMcpController =
        new ListPurchaseOrdersMcpController(mcpParams);
      try {
        const result = await listPurchaseOrdersMcpController.processRequest();
        //return ListPurchaseOrdersMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
