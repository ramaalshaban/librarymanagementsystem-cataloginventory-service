...
class UpdatePurchaseOrderMcpController extends CatalogInventoryMcpController {
  constructor(params) {
    super("updatePurchaseOrder", "updatepurchaseorder", params);
    this.dataName = "purchaseOrder";
    this.crudType = "update";
  }

  createApiManager() {
    return new UpdatePurchaseOrderManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    description:
      "Update status, fulfillment, or approval notes of a branch purchase order.",
    parameters: UpdatePurchaseOrderMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const updatePurchaseOrderMcpController =
        new UpdatePurchaseOrderMcpController(mcpParams);
      try {
        const result = await updatePurchaseOrderMcpController.processRequest();
        //return UpdatePurchaseOrderMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
