...
class UpdateInterBranchTransferMcpController extends CatalogInventoryMcpController {
  constructor(params) {
    super("updateInterBranchTransfer", "updateinterbranchtransfer", params);
    this.dataName = "interBranchTransfer";
    this.crudType = "update";
  }

  createApiManager() {
    return new UpdateInterBranchTransferManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    description:
      "Update status or log of an inter-branch transfer record (status workflow steps, notes, log).",
    parameters: UpdateInterBranchTransferMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const updateInterBranchTransferMcpController =
        new UpdateInterBranchTransferMcpController(mcpParams);
      try {
        const result =
          await updateInterBranchTransferMcpController.processRequest();
        //return UpdateInterBranchTransferMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
