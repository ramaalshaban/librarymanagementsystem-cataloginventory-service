...
class DeleteBookMcpController extends CatalogInventoryMcpController {
  constructor(params) {
    super("deleteBook", "deletebook", params);
    this.dataName = "book";
    this.crudType = "delete";
  }

  createApiManager() {
    return new DeleteBookManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    parameters: DeleteBookMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const deleteBookMcpController = new DeleteBookMcpController(mcpParams);
      try {
        const result = await deleteBookMcpController.processRequest();
        //return DeleteBookMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
